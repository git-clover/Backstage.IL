#!/usr/bin/env bash
#
# Build Backstage.IL Flatpak locally using Docker with the same container image
...
echo "=== Backstage.IL Flatpak Build ==="
...
# ---------- prepare manifest ----------
MANIFEST_SRC="scripts/flatpak/com.orcaslicer.Backstage.IL.yml"
MANIFEST_DOCKER="scripts/flatpak/com.orcaslicer.Backstage.IL.docker.yml"
...
flatpak-builder $FORCE_CLEAN_FLAG \
    ...
    --repo=flatpak-repo \
    flatpak-build \
    scripts/flatpak/com.orcaslicer.Backstage.IL.docker.yml
builder_end=$(date +%s)
