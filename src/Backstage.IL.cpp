...
int CLI::run(int argc, char **argv)
{
    ...
    if (start_gui) {
        ...
        BOOST_LOG_TRIVIAL(info) << "begin to launch Backstage.IL GUI soon";
    ...
    BOOST_LOG_TRIVIAL(warning) << boost::format("cli mode, Current Backstage.IL Version %1%")%SoftFever_VERSION;
...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into Backstage.IL process (for example Nahimic, see GH #5573).
    // We hope that if a DLL is being injected into a Backstage.IL process, it happens at the very start of the application,
    // thus we shall detect them now.
    if (BlacklistedLibraryCheck::get_instance().perform_check()) {
        std::wstring text = L"Following DLLs have been injected into the Backstage.IL process:\n\n";
        text += BlacklistedLibraryCheck::get_instance().get_blacklisted_string();
        text += L"\n\n"
                L"Backstage.IL is known to not run correctly with these DLLs injected. "
                L"We suggest stopping or uninstalling these services if you experience "
                L"crashes or unexpected behaviour while using Backstage.IL.\n"
                L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes Backstage.IL "
...
int main(int argc, char **argv)
{
    ...
    // Backstage.IL with SIGPIPE (exit 141) and produces no crash report.
