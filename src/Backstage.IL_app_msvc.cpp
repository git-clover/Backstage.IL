...
public:
    ...
    bool load_opengl_dll()
    {
        ...
        wc.lpszClassName = L"Backstage.IL_opengl_version_check";
        wc.style = CS_OWNDC;
        if (RegisterClass(&wc)) {
            HWND hwnd = CreateWindowW(wc.lpszClassName, L"Backstage.IL_opengl_version_check", WS_OVERLAPPEDWINDOW, 0, 0, 640, 480, 0, 0, wc.hInstance, (LPVOID)this);
            if (hwnd) {
...
// http://download.qt.io/development_releases/prebuilt/llvmpipe/windows/
    ...
    wcscat(path_to_slic3r, L"Backstage.IL.dll");
//	printf("Loading Slic3r library: %S\n", path_to_slic3r);
    HINSTANCE hInstance_Slic3r = LoadLibraryExW(path_to_slic3r, nullptr, 0);
    if (hInstance_Slic3r == nullptr) {
        printf("Backstage.IL.dll was not loaded, error=%d\n", GetLastError());
        return -1;
    }
    ...
    if (orcaslicer_main == nullptr) {
        printf("could not locate the function orcaslicer_main in Backstage.IL.dll\n");
        return -1;
    }
