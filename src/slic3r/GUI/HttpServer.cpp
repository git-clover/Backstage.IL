...
std::shared_ptr<HttpServer::Response> HttpServer::auth_handle_request(const std::string& url, const std::string& provider)
{
    ...
    if (!auth_code.empty()) {
        ...
        const std::string message = login_ok
            ? "You can return to Backstage.IL. This window will close automatically."
            : "Something went wrong. Please return to Backstage.IL and try again.";
...
void HttpServer::ResponseRedirect::write_response(std::stringstream& ssOut)
{
    const std::string sHTML =
        ...
        "<p>You can return to Backstage.IL. If your browser does not redirect automatically, use the button below.</p>"
