...
public:
        ...
        )
    {
        ...
        m_progress_fg_color = StateColor::darkModeColorFor(wxColour("#004fbd"));
...
static void migrate_flatpak_legacy_datadir(const boost::filesystem::path &data_dir_path)
{
    ...
    boost::replace_first(legacy_data_dir_str, "com.orcaslicer.Backstage.IL", "io.github.softfever.Backstage.IL");
...
/*#ifdef WIN32
    ...
    try {
        ...
        flush_logs();
        wxString errmsg = wxString::Format(_L("Backstage.IL will terminate because of running out of memory. "
        ...
        //throw;
     	...
     	BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        	flush_logs();
        wxString errmsg = _L("Backstage.IL will terminate because of a localization error. "
        ...
        BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        flush_logs();
        wxLogError(format_wxstr(_L("Backstage.IL got an unhandled exception: %1%"), ex.what()));
        throw;
    }
//#endif
}
    ...
    if (is_editor() && m_last_config_version && m_last_config_version->valid()
        ...
        CallAfter([] {
            ...
            MessageDialog dlg(nullptr,
                _L("Since version 2.4.0, Backstage.IL syncs user profiles through Orca Cloud instead of Bambu Cloud.\n\n"
                   "To migrate your existing profiles, log in to Orca Cloud and they will be transferred automatically. "
                   "To learn more about how Backstage.IL stores and syncs your profiles, or to migrate your presets manually, check out our wiki.\n\n"
...
{
	//app config initializes early becasuse it is used in instance checking in Backstage.IL.cpp
...
static boost::optional<Semver> parse_semver_from_ini(std::string path)
{
    ...
    size_t start = body.find("Backstage.IL ");
    if (start == std::string::npos) {
        start = body.find("Backstage.IL ");
        if (start == std::string::npos)
            return boost::none;
    }
...
//	SetAppDisplayName(SLIC3R_APP_NAME);
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("gui mode, Current Backstage.IL Version %1% build %2%") % SoftFever_VERSION % GIT_COMMIT_HASH;
...
//     Slic3r::debugf "wxWidgets version %s, Wx version %s\n", wxVERSION_STRING, wxVERSION;
        ...
        if (!msg.empty() && !ssl_accept) {
            RichMessageDialog
                dlg(nullptr,
                    ...
                    "Backstage.IL", wxICON_QUESTION | wxYES_NO);
    ...
    //Orca: write Backstage.IL version
    ...
    if (is_editor()) {
        ...
        Bind(EVT_ENTER_FORCE_UPGRADE, [this](const wxCommandEvent& evt) {
                ...
                DownloadDialog dialog(this->mainframe,
                    tips,
                    _L("Backstage.IL needs an update"),
                    false,
                    wxCENTER | wxICON_INFORMATION);
...
//#endif
    ...
    if (m_config_corrupted) {
        ...
        show_error(nullptr,
                   _u8L(
                       "The Backstage.IL configuration file may be corrupted and cannot be parsed.\nBackstage.IL has attempted to recreate the "
...
void GUI_App::UpdateDarkUI(wxWindow* window, bool highlited/* = false*/, bool just_font/* = false*/)
{
    ...
    if (m_is_dark_mode) {
        ...
        // * the background color of a button: #004fbd  -- 73
        if (bg_col != orig_col) {
...
bool GUI_App::process_network_msg(std::string dev_id, std::string msg)
{
    if (dev_id.empty()) {
        if (msg == "wait_info") {
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("Please try updating Backstage.IL and then try again."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("The certificate has expired. Please check the time settings or update Backstage.IL and try again."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("Internal error. Please try upgrading the firmware and Backstage.IL version. If the issue persists, contact support."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            if (!m_show_error_msgdlg && !m_unsigned_plugin_warning_shown) {
                ...
                MessageDialog
                    msg_dlg(nullptr,
                            _L("To use Backstage.IL with Bambu Lab printers, you need to enable LAN mode and Developer mode on your printer.\n\n"
                               "Please go to your printer's settings and:\n"
                               "1. Turn on LAN mode\n"
                               "2. Enable Developer mode\n\n"
                               "Developer mode allows the printer to work exclusively through local network access, "
                               "enabling full functionality with Backstage.IL."),
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (initial) {
        ...
        if (! language.empty())
        	BOOST_LOG_TRIVIAL(info) << boost::format("language provided by Backstage.IL.conf: %1%") % language;
        else {
	...
	if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
    	BOOST_LOG_TRIVIAL(trace) << boost::format("The following language code requires right to left layout, which is not supported by Backstage.IL: %1%") % language_info->CanonicalName.ToUTF8().data();
		language_info = nullptr;
	}
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString &fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening PrusaSlicer by drag & dropping a G-Code onto Backstage.IL icon in Finder,
...
void GUI_App::associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id = L" Orca.Slicer.1";
    std::wstring prog_desc = L"Backstage.IL";
