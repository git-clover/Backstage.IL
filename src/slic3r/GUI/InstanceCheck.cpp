...
namespace instance_check_internal
{
	...
	static BOOL CALLBACK EnumWindowsProc(_In_ HWND   hwnd, _In_ LPARAM lParam)
	{
		...
		// properties are missing, it is not an Backstage.IL main window.
	...
	static bool  send_message(const std::string &message_text, const std::string &version)
	{
		...
		if ( !checker->IsAnotherRunning() ) */
		{
			...
			std::string		interface_name = "com.orcaslicer.Backstage.IL.InstanceCheck.Object" + version;
			std::string   	method_name = "AnotherInstance";
			std::string		object_name = "/com/orcaslicer/Backstage.IL/InstanceCheck/Object" + version;
...
namespace MessageHandlerDBusInternal
{
	...
	static void respond_to_introspect(DBusConnection *connection, DBusMessage *request) 
	{
	    ...
	    const char  *introspection_data =
	        ...
	        "     </method>"
	        "   </interface>"
	        "   <interface name=\"com.orcaslicer.Backstage.IL.InstanceCheck\">"
	...
	//method AnotherInstance receives message from another Backstage.IL instance 
	...
	static DBusHandlerResult handle_dbus_object_message(DBusConnection *connection, DBusMessage *message, void *user_data)
	{
	    ...
	    std::string our_interface  = "com.orcaslicer.Backstage.IL.InstanceCheck.Object" + wxGetApp().get_instance_hash_string();
...
void OtherInstanceMessageHandler::listen()
{
	...
	std::string			 interface_name = "com.orcaslicer.Backstage.IL.InstanceCheck.Object" + instance_hash;
	std::string			 object_name 	= "/com/orcaslicer/Backstage.IL/InstanceCheck/Object" + instance_hash;
	...
	if (DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER != name_req_val) {
		BOOST_LOG_TRIVIAL(error) << "Not primary owner of DBus name - probably another Backstage.IL instance is running.";
	    BOOST_LOG_TRIVIAL(error) << "Dbus Messages listening terminating.";
	    dbus_connection_unref(conn);
	    return;
	}
