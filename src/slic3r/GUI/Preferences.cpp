...
private:
    void OnPaint(wxPaintEvent& evt)
    {
        ...
        wxColour textCol = StateColor::darkModeColorFor(m_hovered ? "#0061E9" : "#363636");
...
wxBoxSizer* PreferencesDialog::create_item_link_association( wxString url_prefix, wxString website_name)
{
    ...
    wxString tooltip = _L("Associate") + " " + url_prefix + ":// " + _L("with Backstage.IL so that Orca can open models from") + " " + website_name;
...
void PreferencesDialog::create_items()
{
    ...
    auto item_single_instance  = create_item_checkbox(_L("Allow only one Backstage.IL instance"),
    #if __APPLE__
    ...
    #else
            _L("If this is enabled, when starting Backstage.IL and another instance of the same Backstage.IL is already running, that instance will be reactivated instead."),
    #endif
            "single_instance");
    ...
    auto item_auto_reslice = create_item_auto_reslice(
        ...
        _L("If enabled, Backstage.IL will re-slice automatically whenever slicing-related settings change."),
    ...
    if (is_running_in_msix()) {
        ...
        g_sizer->Add(create_item_title(_L("Associate files to Backstage.IL")), 1, wxEXPAND);
    ...
    auto item_associate_3mf    = create_item_checkbox(_L("Associate 3MF files to Backstage.IL"), _L("If enabled, this sets Backstage.IL as the default application to open 3MF files.") , "associate_3mf");
    g_sizer->Add(item_associate_3mf);

    auto item_associate_drc = create_item_checkbox(_L("Associate DRC files to Backstage.IL"), _L("If enabled, sets Backstage.IL as default application to open DRC files."), "associate_drc");
    g_sizer->Add(item_associate_drc);

    auto item_associate_stl    = create_item_checkbox(_L("Associate STL files to Backstage.IL"), _L("If enabled, this sets Backstage.IL as the default application to open STL files.") , "associate_stl");
    g_sizer->Add(item_associate_stl);

    auto item_associate_step   = create_item_checkbox(_L("Associate STEP files to Backstage.IL"), _L("If enabled, this sets Backstage.IL as the default application to open STEP files."), "associate_step");
    g_sizer->Add(item_associate_step);

    //// ASSOCIATE > WebLinks
    g_sizer->Add(create_item_title(_L("Associate web links to Backstage.IL")), 1, wxEXPAND);
