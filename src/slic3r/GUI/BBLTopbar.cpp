    ...
    : wxControl()
{
    ...
    Bind(wxEVT_PAINT, [this](wxPaintEvent&) {
        ...
        dc.SetBackground(wxBrush(wxColour(0, 13, 31)));
        dc.Clear();
...
{
    dc.SetBrush(wxBrush(wxColour(0, 13, 31)));
...
void BBLTopbarArt::DrawButton(wxDC& dc, wxWindow* wnd, const wxAuiToolBarItem& item, const wxRect& rect)
{
    ...
    if (!(item.GetState() & wxAUI_BUTTON_STATE_DISABLED))
    {
        ...
        {
            dc.SetPen(wxPen(StateColor::darkModeColorFor("#003887"))); // ORCA
            dc.SetBrush(wxBrush(StateColor::darkModeColorFor("#003887"))); // ORCA
            dc.DrawRectangle(rect);
        }
