...
void GCodeViewer::render_legend_color_arr_recommen(float window_padding)
{
    ...
    auto link_text = [&](const std::string &label) {
        ...
        ImColor HyperColor = ImColor(0, 79, 189, 255).Value;
    ...
    auto link_filament_group_wiki = [&](const std::string& label) {
        ImVec2 wiki_part_size = ImGui::CalcTextSize(label.c_str());
        ImColor HyperColor = ImColor(0, 79, 189, 255); // ORCA match color
        ImGui::PushStyleColor(ImGuiCol_Text, HyperColor.Value);
        imgui.text(label.c_str());
        ImGui::PopStyleColor();
...
void GCodeViewer::push_combo_style()
{
    ...
    ImGui::PushStyleColor(ImGuiCol_BorderActive, ImVec4(0.00f, 0.31f, 0.74f, 1.00f));
    ImGui::PushStyleColor(ImGuiCol_HeaderHovered, ImVec4(0.00f, 0.31f, 0.74f, 0.0f));
