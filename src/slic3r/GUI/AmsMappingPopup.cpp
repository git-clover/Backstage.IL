...
void MaterialItem::doRender(wxDC& dc)
{
    ...
    if (m_selected)
    {
        dc.SetPen(wxPen(wxColour("#003887"), FromDIP(2))); // ORCA selected item border color (Orca accent)
        dc.SetBrush(*wxTRANSPARENT_BRUSH);
...
void MaterialSyncItem::doRender(wxDC &dc)
{
    ...
    if (m_selected) {
        dc.SetPen(wxColour("#003887")); // ORCA selected item border color (Orca accent)
        dc.SetBrush(*wxTRANSPARENT_BRUSH);
        dc.DrawRoundedRectangle(1, 1, size.x - 1, size.y - 1, 5);
    }
#else
...
AmsMapingPopup::AmsMapingPopup(wxWindow *parent, bool use_in_sync_dialog) :
    PopupWindow(parent, wxBORDER_NONE | wxPU_CONTAINS_CONTROLS), m_use_in_sync_dialog(use_in_sync_dialog)
 {
     ...
     m_title_text->SetForegroundColour(wxColour("#000d1f"));
     ...
     m_left_tips->SetForegroundColour(StateColor::darkModeColorFor("0x000d1f"));
     ...
     m_right_tips->SetForegroundColour(StateColor::darkModeColorFor("0x000d1f"));
...
{
    wxPaintDC dc(this);
    dc.SetPen(wxPen(StateColor::darkModeColorFor(wxColour("#003887")), FromDIP(2))); // Orca: colorful accent border for separation (dark-mode aware)
    dc.SetBrush(*wxTRANSPARENT_BRUSH);
...
void AmsReplaceMaterialDialog::create()
{
    ...
    identical_filament->SetForegroundColour(StateColor::darkModeColorFor(wxColour("#003887"))); // Orca: accent teal (not brand green)
    ...
    StateColor btn_bg_green(std::pair<wxColour, int>(wxColour(0, 92, 213), StateColor::Pressed), std::pair<wxColour, int>(wxColour(0, 55, 133), StateColor::Normal)); // Orca: accent teal button (not brand green)
...
void AmsRMGroup::doRender(wxDC& dc)
{
    ...
    for (auto iter = m_group_info.rbegin(); iter != m_group_info.rend(); ++iter) {
        ...
        dc.SetTextForeground(tray_color.GetLuminance() < 0.6 ? *wxWHITE : wxColour("#000d1f"));
        if (tray_color.Alpha() == 0) { dc.SetTextForeground(wxColour("#000d1f")); }
