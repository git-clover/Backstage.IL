...
void MaterialItem::doRender(wxDC& dc)
{
    ...
    if (m_selected)
    {
        dc.SetPen(wxPen(wxColour("#004fbd"), FromDIP(2))); // ORCA selected item border color (Orca accent)
        dc.SetBrush(*wxTRANSPARENT_BRUSH);
...
void MaterialSyncItem::doRender(wxDC &dc)
{
    ...
    if (m_selected) {
        dc.SetPen(wxColour("#004fbd")); // ORCA selected item border color (Orca accent)
        dc.SetBrush(*wxTRANSPARENT_BRUSH);
        dc.DrawRoundedRectangle(1, 1, size.x - 1, size.y - 1, 5);
    }
#else
...
{
    wxPaintDC dc(this);
    dc.SetPen(wxPen(StateColor::darkModeColorFor(wxColour("#004fbd")), FromDIP(2))); // Orca: colorful accent border for separation (dark-mode aware)
    dc.SetBrush(*wxTRANSPARENT_BRUSH);
...
void AmsReplaceMaterialDialog::create()
{
    ...
    // set icon for dialog
    std::string icon_path = (boost::format("%1%/images/Backstage.ILTitle.ico") % resources_dir()).str();
    ...
    identical_filament->SetForegroundColour(StateColor::darkModeColorFor(wxColour("#004fbd"))); // Orca: accent teal (not brand green)
