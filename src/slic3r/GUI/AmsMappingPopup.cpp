...
void MaterialItem::doRender(wxDC& dc)
{
    ...
    if (m_selected)
    {
        dc.SetPen(wxPen(wxColour("#002b6b"), FromDIP(2))); // ORCA selected item border color (Orca accent)
        dc.SetBrush(*wxTRANSPARENT_BRUSH);
...
void MaterialSyncItem::doRender(wxDC &dc)
{
    ...
    if (m_selected) {
        dc.SetPen(wxColour("#002b6b")); // ORCA selected item border color (Orca accent)
        dc.SetBrush(*wxTRANSPARENT_BRUSH);
        dc.DrawRoundedRectangle(1, 1, size.x - 1, size.y - 1, 5);
    }
#else
...
{
    wxPaintDC dc(this);
    dc.SetPen(wxPen(StateColor::darkModeColorFor(wxColour("#002b6b")), FromDIP(2))); // Orca: colorful accent border for separation (dark-mode aware)
    dc.SetBrush(*wxTRANSPARENT_BRUSH);
...
void AmsReplaceMaterialDialog::create()
{
    ...
    identical_filament->SetForegroundColour(StateColor::darkModeColorFor(wxColour("#002b6b"))); // Orca: accent teal (not brand green)
