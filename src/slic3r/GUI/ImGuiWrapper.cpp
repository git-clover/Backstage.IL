...
const ImVec4 ImGuiWrapper::COL_GREEN_LIGHT       = { 0   / 255.f, 79  / 255.f, 189 / 255.f, 0.25f }; // ORCA used on various places like text selection bg. Replaced with orca color
...
bool ImGuiWrapper::bbl_slider_float_style(const std::string &label, float *v, float v_min, float v_max, const char *format, float power, bool clamp, const wxString &tooltip)
{
    ...
    ImGui::PushStyleColor(ImGuiCol_SliderGrabActive, ImVec4(0.00f, 0.31f, 0.74f, 1.00f));
...
bool ImGuiWrapper::bbl_checkbox(const wxString &label, bool &value)
{
    ...
    bool b_value = value;
    if (b_value) {
        ImGui::PushStyleColor(ImGuiCol_FrameBg, ImVec4(0.00f, 0.31f, 0.74f, 1.00f));
        ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImVec4(0.00f, 0.31f, 0.74f, 1.00f));
        ImGui::PushStyleColor(ImGuiCol_FrameBgActive, ImVec4(0.00f, 0.31f, 0.74f, 1.00f));
    }
    auto label_utf8 = into_u8(label);
...
void ImGuiWrapper::push_menu_style(const float scale)
{
    if (m_is_dark_mode) {
        ...
        ImGui::PushStyleColor(ImGuiCol_Header, ImVec4(0.00f, 0.31f, 0.74f, 1.0f));
        ImGui::PushStyleColor(ImGuiCol_HeaderHovered, ImVec4(0.00f, 0.31f, 0.74f, 1.0f));
        ImGui::PushStyleColor(ImGuiCol_HeaderActive, ImVec4(0.00f, 0.31f, 0.74f, 1.0f));
    }
    else {
        ImGuiWrapper::push_toolbar_style(scale);
        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(10.0f, 10.0f) * scale);
...
void ImGuiWrapper::push_common_window_style(const float scale) {
    if (m_is_dark_mode) {
        ...
        ImGui::PushStyleColor(ImGuiCol_Separator, ImGuiWrapper::COL_SEPARATOR_DARK);                             // 5
        ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(1.00f, 1.00f, 1.00f, 1.00f));                              // 6
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.00f, 0.31f, 0.74f, 1.00f));                       // 7
        ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0.00f, 0.31f, 0.74f, 1.00f));                        // 8
        ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImVec4(84 / 255.0f, 84 / 255.0f, 90 / 255.0f, 1.00f));    // 9
        ImGui::PushStyleColor(ImGuiCol_FrameBgActive, ImVec4(62 / 255.0f, 62 / 255.0f, 69 / 255.0f, 1.00f));     // 10
        ImGui::PushStyleColor(ImGuiCol_FrameBg, ImVec4(238 / 255.0f, 238 / 255.0f, 238 / 255.0f, 0.00f));        // 11
        ...
        ImGui::PushStyleColor(ImGuiCol_PlotHistogram, ImVec4(0.00f, 0.31f, 0.74f, 1.00f));                       // 14
    }
    else {
        ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize, 1.0f * scale);
        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(20.0f, 10.0f) * scale);
...
void ImGuiWrapper::push_confirm_button_style() {
    if (m_is_dark_mode) {
        ...
        ImGui::PushStyleColor(ImGuiCol_Button,        to_ImVec4(decode_color_to_float_array("#004fbd")));
        ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(0.f / 255.f, 79 / 255.f, 189 / 255.f, 1.f));
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, to_ImVec4(decode_color_to_float_array("#0061E9")));
        ImGui::PushStyleColor(ImGuiCol_ButtonActive,  to_ImVec4(decode_color_to_float_array("#004fbd")));
...
void ImGuiWrapper::push_combo_style(const float scale)
{
    if (m_is_dark_mode) {
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize, 1.0f * scale);
        ImGui::PushStyleColor(ImGuiCol_PopupBg, ImGuiWrapper::COL_WINDOW_BG_DARK);
        ImGui::PushStyleColor(ImGuiCol_BorderActive, ImVec4(0.f, 79 / 255.f, 189 / 255.f, 0.6f));  // ORCA hovered item border color
        ...
        ImGui::PushStyleColor(ImGuiCol_Header, ImVec4(0.f, 79 / 255.f, 189 / 255.f, 0.25f));       // ORCA active item background color
...
void ImGuiWrapper::push_radio_style(const float scale)
{
    ...
    if (m_is_dark_mode) {
        ImGui::PushStyleColor(ImGuiCol_CheckMark, to_ImVec4(decode_color_to_float_array("#004fbd"))); // ORCA use orca color for radio buttons
