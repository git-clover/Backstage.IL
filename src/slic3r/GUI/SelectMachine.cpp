    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    m_ams_backup_tip->SetForegroundColour(wxColour("#002b6b"));
    ...
    m_link_edit_nozzle->SetForegroundColour("#002b6b");
...
{
    if (enable) {
        m_ams_backup_tip->SetForegroundColour(StateColor::darkModeColorFor("#002b6b"));
    }
    else {
...
void PrintOptionItem::doRender(wxDC& dc)
{
    ...
    for (auto it = m_ops.begin(); it != m_ops.end(); ++it)
    {
        ...
        {
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#002b6b") : StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
