    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    m_ams_backup_tip->SetForegroundColour(wxColour("#003887"));
    ...
    m_link_edit_nozzle->SetForegroundColour("#003887");
...
{
    if (enable) {
        m_ams_backup_tip->SetForegroundColour(StateColor::darkModeColorFor("#003887"));
    }
    else {
...
void PrintOptionItem::doRender(wxDC& dc)
{
    ...
    for (auto it = m_ops.begin(); it != m_ops.end(); ++it)
    {
        ...
        {
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#003887") : StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
