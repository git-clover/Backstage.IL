...
// This block of code is no longer used. I'll comment it out instead.
/* void KBShortcutsDialog::OnSelectTabel(wxCommandEvent &event)
{
    ...
    while (i != m_hash_selector.end()) {
        Select *sel = i->second;
        if (id == sel->m_index) {
            sel->m_tab_button->SetBackgroundColour(StateColor::darkModeColorFor(wxColour("#A9BCD8"))); // ORCA color for selected tab background
            sel->m_tab_text->SetBackgroundColour(StateColor::darkModeColorFor(wxColour("#A9BCD8"))); // ORCA color for selected tab background
...
wxWindow *KBShortcutsDialog::create_button(int id, wxString text)
{
    ...
    stext->SetForegroundColour(wxColour(38, 51, 69));
    stext->Wrap(-1);
