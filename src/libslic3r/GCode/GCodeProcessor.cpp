...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    //BBS: Backstage.IL is also "bambu". Otherwise the time estimation didn't work.
    //FIXME: Workaround and should be handled when do removing-bambu
    { EProducer::Backstage.IL, SLIC3R_APP_NAME },
    { EProducer::Backstage.IL, "generated by Backstage.IL" },
    { EProducer::Backstage.IL, "generated by BambuStudio" },
    { EProducer::Backstage.IL, "BambuStudio" }
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by Backstage.IL,
        // extract the config from it
        if (m_producer == EProducer::Backstage.IL || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from Backstage.IL's own G-code.
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::Backstage.IL) {
        // height tag
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::Backstage.IL: { return process_bambuslicer_tags(comment); }
