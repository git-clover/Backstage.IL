...
// Backstage.IL version tag
const std::string ORCASLICER_TAG                    = "Backstage.IL";
...
#define _(s) Slic3r::I18N::translate(s)
    ...
    class _BBS_3MF_Importer : public _BBS_3MF_Base
    {
        ...
        // Semantic version from the Backstage.IL metadata tag (if present).
        ...
        ConfigSubstitutionContext& config_substitutions, LoadStrategy strategy, bool* is_bbl_3mf, bool* is_orca_3mf, Semver& file_version, Import3mfProgressFn proFn, BBLProject *project, int plate_id)
    {
        ...
        // If the Backstage.IL tag is present, use it as file_version (ignoring the Bambu Application version).
        ...
        int plate_id)
    {
        ...
        if (!m_is_bbl_3mf) {
            // if the 3mf was not produced by Backstage.IL and there is more than one instance,
...
//        model.adjust_min_z();
    ...
    bool _BBS_3MF_Importer::_handle_end_model()
    {
        ...
        if (!m_is_bbl_3mf) {
            // if the 3mf was not produced by Backstage.IL and there is only one object,
    ...
    bool _BBS_3MF_Importer::_handle_end_metadata()
    {
        if ((m_curr_metadata_name == BBS_3MF_VERSION)||(m_curr_metadata_name == BBS_3MF_VERSION1)) {
            ...
            else if (boost::starts_with(m_curr_characters, "Backstage.IL-")) {
                m_is_bbl_3mf = true;
                m_bambuslicer_generator_version = Semver::parse(m_curr_characters.substr(11));
            }
        } else if (m_curr_metadata_name == ORCASLICER_TAG) {
            // Backstage.IL version tag (written from Backstage.IL 2.3.2 onwards)
        ...
        /*} else if (m_curr_metadata_name == BBS_FDM_SUPPORTS_PAINTING_VERSION) {
            ...
            check_painting_version(m_fdm_supports_painting_version, FDM_SUPPORTS_PAINTING_VERSION,
                _(L("The selected 3MF contains FDM supports painted object using a newer version of Backstage.IL and is not compatible.")));
        } else if (m_curr_metadata_name == BBS_SEAM_PAINTING_VERSION) {
            m_seam_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_seam_painting_version, SEAM_PAINTING_VERSION,
                _(L("The selected 3MF contains seam painted object using a newer version of Backstage.IL and is not compatible.")));
        } else if (m_curr_metadata_name == BBS_MM_PAINTING_VERSION) {
            m_mm_painting_version = (unsigned int) atoi(m_curr_characters.c_str());
            check_painting_version(m_mm_painting_version, MM_PAINTING_VERSION,
                _(L("The selected 3MF contains multi-material painted object using a newer version of Backstage.IL and is not compatible.")));*/
    ...
    bool _BBS_3MF_Importer::_generate_volumes(ModelObject& object, const Geometry& geometry, const ObjectMetadata::VolumeMetadataList& volumes, ConfigSubstitutionContext& config_substitutions)
    {
        ...
        for (const ObjectMetadata::VolumeMetadata& volume_data : volumes) {
            ...
            if (!m_is_bbl_3mf) {
                // if the 3mf was not produced by Backstage.IL and there is only one instance,
                // bake the transformation into the geometry to allow the reload from disk command
                // to work properly
                if (object.instances.size() == 1) {
                    triangle_mesh.transform(object.instances.front()->get_transformation().get_matrix(), false);
    ...
    */
        ...
        int export_plate_idx)
    {
        ...
        // The content of this file is the same for each Backstage.IL 3mf.
    ...
    */
    ...
    bool _BBS_3MF_Exporter::_add_slice_info_config_file_to_archive(mz_zip_archive& archive, const Model& model, PlateDataPtrs& plate_data_list, const ObjectToObjectDataMap &objects_data, const DynamicPrintConfig& config)
    {
        ...
        stream << "    <" << SLICE_HEADER_ITEM_TAG << " " << KEY_ATTR << "=\"" << "Backstage.IL-Version" << "\" " << VALUE_ATTR << "=\"" << SoftFever_VERSION << "\"/>\n";
        stream << "  </" << SLICE_HEADER_TAG << ">\n";
