        ...
        const ToolOrdering          &tool_ordering)
    {
	    ...
	    if (! extruders.empty()) {
	        ...
	        for (const Extruder &extruder : extruders) {
	            ...
	            double extruded_volume = extruder.extruded_volume() + (has_wipe_tower ? wipe_tower_data.used_filament[extruder.id()] * 2.4052f : 0.f); // assumes 1.75 mmfilament diameter
    ...
    const bool                               prime_extruder)
{
    ...
    if (!m_config.layer_change_gcode.value.empty()) {
        ...
        for (const Extruder &extruder : m_writer.extruders()) {
            double extruded_volume = extruder.extruded_volume() + (has_wipe_tower ? print.wipe_tower_data().used_filament[extruder.id()] * 2.4052f : 0.f); // assumes 1.75 mmfilament diameter
            double filament_weight = extruded_volume * extruder.filament_density() * 0.001;
            if (filament_weight > 0.)
