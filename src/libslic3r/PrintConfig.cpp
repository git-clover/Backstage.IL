...
void PrintConfigDef::init_fff_params()
{
    ...
    def->tooltip = L(
        ...
        "set appropriately. Backstage.IL ensures that adaptive_bed_mesh_min/adaptive_bed_mesh_max values do not exceed these min/max "
    ...
    */
    ...
    def->tooltip = L("Do not write the CONFIG_BLOCK (slicer configuration key/value pairs) into the G-code file. "
                   ...
                   "so importing it back into Backstage.IL will not restore the configuration.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
...
CLIMiscConfigDef::CLIMiscConfigDef()
{
    ...
    def->tooltip = L("This version of Backstage.IL may not understand configurations produced by the newest Backstage.IL versions. "
                     "For example, newer Backstage.IL may extend the list of supported firmware flavors. One may decide to "
    ...
    def->tooltip = L("If enabled, the command line arguments are sent to an existing instance of GUI Backstage.IL, "
                     "or an existing Backstage.IL window is activated. "
...
ReadWriteSlicingStatesConfigDef::ReadWriteSlicingStatesConfigDef()
{
    ...
    def->tooltip = L("Position of the extruder at the beginning of the custom G-code block. If the custom G-code travels somewhere else, "
                     "it should write to this variable so Backstage.IL knows where it travels from when it gets control back.");
    ...
    def->tooltip = L("Retraction state at the beginning of the custom G-code block. If the custom G-code moves the extruder axis, "
                     "it should write to this variable so Backstage.IL de-retracts correctly when it gets control back.");
