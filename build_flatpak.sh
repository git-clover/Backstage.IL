...
# Backstage.IL Flatpak Build Script
# This script builds and packages Backstage.IL as a Flatpak package locally
...
show_help() {
    ...
    echo ""
    echo "Build Backstage.IL as a Flatpak package"
    echo ""
    echo "Options:"
...
echo -e "${BLUE}Backstage.IL Flatpak Build Script${NC}"
echo -e "${BLUE}================================${NC}"
...
if [[ ! -f "./scripts/flatpak/com.orcaslicer.Backstage.IL.yml" ]]; then
    echo -e "${RED}Error: Flatpak manifest not found at scripts/flatpak/com.orcaslicer.Backstage.IL.yml${NC}"
    exit 1
fi
...
BUNDLE_NAME="Backstage.IL-Linux-flatpak_${VER}_${ARCH}.flatpak"
...
MANIFEST="scripts/flatpak/com.orcaslicer.Backstage.IL.yml"
if [[ "$NO_DEBUGINFO" == true ]]; then
    MANIFEST="scripts/flatpak/com.orcaslicer.Backstage.IL.no-debug.yml"
    sed '/^build-options:/a\  no-debuginfo: true\n  strip: true' \
        scripts/flatpak/com.orcaslicer.Backstage.IL.yml > "$MANIFEST"
...
if ! flatpak-builder \
    ...
    rm -f "scripts/flatpak/com.orcaslicer.Backstage.IL.no-debug.yml"
    exit 1
fi
...
echo ""
echo -e "${BLUE}To run Backstage.IL:${NC}"
echo -e "flatpak run com.orcaslicer.Backstage.IL"
echo ""
echo -e "${BLUE}To uninstall:${NC}"
echo -e "flatpak uninstall --user com.orcaslicer.Backstage.IL"
echo ""
if [[ "$FORCE_CLEAN" != true ]]; then
